...
local PlayerWhitelistCheck = function(Player, UseStatic, Code)
	local suc, err = pcall(function()
		-- keep the old hash available for backwards compatibility
		local PlayerHash = HashLib and HashLib.sha1 and HashLib.sha1(tostring(Player.Name..Player.UserId)) or nil
		-- new-format key: concatenation of Name and UserId (string)
		...
		if Members then
			-- Members may be:
			-- 1) an array of subtables like { "NameUserId", 12345 }
			-- 2) an array of strings (legacy): either raw "NameUserId" or SHA1 hash
			for _, entry in ipairs(Members) do
				if type(entry) == "table" then
					-- expected new format: { "NameUserId", UserId }
					if entry[1] == PlayerKey or (entry[2] and tonumber(entry[2]) == PlayerId) then
						PrivateMembers[Player.Name] = true
						break
					end
				elseif type(entry) == "string" then
					-- legacy string entry: could be raw Name..UserId or SHA1 hash
...
local WLNumber = math.random(1, 10000)
	...
	EnabledGodMode = function(NoNotifacations)
		...
		if AvailableSeat then
			...
			if Functions:IsDisablerOn() then
				...
				WindUI:Notify({
					Title = "God Mode",
					Content = "Success in " .. TimeInSecs ..  "s ! Disabled TP Check!",
					Duration = 10,
				})
			...
			while _G.AutoRob and task.wait() do
				if not Functions:IsDisablerOn() then
					...
					if not GodModeEnabled then
						...
						if not GodModeEnabled then
							WindUI:Notify({
								Title = "Auto Disable TP Check",
								Content = "Failed to auto enable Disable TP Check.",
								Duration = 5,
							})
						end
					end
				...
				if not Functions:IsDisablerOn() then
					_G.PushNotification("Yellow", "Please enable Disable TP Check")
					continue
				end
	...
	Tabs.Robberies:Button({
		...
		Desc = "Teleports you to an Empty Server that No one can join (Useful for AutoRobing without getting PRC on u)",
		Callback = function()
...
--[[	Tabs.Private:Section({	Title = "Custom Scripts" })
	...
	Tabs.Private:Button({
		...
		Callback = function()
			WindUI:Notify({ Title = "Copy Server Assets", Content = "Fetching...", Duration = 2 })
	...
	Tabs.Private:Button({
		...
		Callback = function()
			WindUI:Notify({ Title = "Show All V1", Content = "Fetching...", Duration = 2 })

			local ok, body = pcall(function()
				if typeof(game.HttpGet) == "function" then
					return game:HttpGet(GITHUB_RAW2)
				else
					return HttpGetAsync and HttpGetAsync(GITHUB_RAW2) or error("HttpGet not available")
				end
			end)
